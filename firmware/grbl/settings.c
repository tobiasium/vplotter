...
void settings_restore(uint8_t restore_flag) {  
  if (restore_flag & SETTINGS_RESTORE_DEFAULTS) {
	...
	settings.motordistance = DEFAULT_MOTORDISTANCE;
    settings.x0 = DEFAULT_X0;
    settings.y0 = DEFAULT_Y0;
...
uint8_t settings_store_global_setting(uint8_t parameter, float value) {
  ...
  if (parameter >= AXIS_SETTINGS_START_VAL) {
    ...
    switch(parameter) {
      ...
      case 28: settings.motordistance = value; break;
      case 29: settings.x0 = value; break;
      case 30: settings.y0 = value; break;
      default: 
