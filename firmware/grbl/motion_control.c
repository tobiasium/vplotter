...
#endif
{
  ...
  //#else
    ...
    float motordistance = settings.motordistance;  // 1148;
    float x0 = settings.x0;  // 307;
    float y0 = settings.y0;  // 1112;
    float a0 = sqrt(labs(x0*x0 + y0*y0 ));
    float b0 = sqrt(labs((motordistance-x0)*(motordistance-x0) + y0*y0 ));
    
    float xa = x0 + target[X_AXIS];
    float xb = motordistance - x0 - target[X_AXIS];
    float y  = y0 - target[Y_AXIS];
    target_polar[X_AXIS] = sqrt(labs(xa*xa+y*y)) - a0;
    target_polar[Y_AXIS] = sqrt(labs(xb*xb+y*y)) - b0;
...
#endif
{
  float mm_per_line_segment=30;  //TODO: move to settings was 2, trying bigger numbers
  ...
  if (segments) {
      ...
      uint16_t i;
	  for (i = 0; i<segments; i++) { // Increment (segments-1). i = 0 statt i=1 start
